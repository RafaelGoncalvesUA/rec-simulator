...
def check_action(env, action):
    control_dict = env.get_action(action)

    success_controls["battery_discharge"] += int(control_dict["battery_discharge"] > 0)
    success_controls["battery_charge"] += int(control_dict["battery_charge"] > 0)
    success_controls["grid_import"] += int(control_dict["grid_import"] > 0)
    success_controls["grid_export"] += int(control_dict["grid_export"] > 0)

while not microgrid_env.done:
    action, _ = agent.predict(obs, deterministic=False)

    check_action(microgrid_env, action)
...
print("Successful controls:", success_controls)
