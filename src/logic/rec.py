...
class RenewableEnergyCommunity:
    def __init__(self, rec_id, market, marginal_price_ts=None):
        """Initialises a Renewable Energy Community (REC).

        Args:
            rec_id (int): Unique ID of the REC.
            market (object): External market interface (e.g., P2P or OMIE).
            marginal_price_ts (pd.DataFrame or None): Time series of marginal prices for each timestep.
        """
        self.rec_id = rec_id
        self.cost = 0
    ...
    def negotiate(self, energy_need):
        """Submits a bid to the external market for buying or selling energy.

        Args:
            energy_need (float): Net energy the REC wants to buy ( >0) or sell (<0).
        """
    ...
    def handle_exportations(self):
        """Processes energy exports from tenants into the shared energy pool.

        Returns:
            float: Total energy shortfall for the REC after pooling tenant surpluses.
        """
    ...
    def execute_tenant_transactions(self):
        """Executes energy import/export actions for each tenant and updates observations and costs.
        """
        for tenant_id in range(self.n_tenants):
            env = self.environments[tenant_id]
    ...
    def handle_market_transactions(self, transactions):
        """Processes the result of market negotiation and updates energy pool and costs.

        Args:
            transactions (pd.DataFrame): Market-cleared transactions affecting the REC.
        """
    ...
    def handle_importations(self):
        """Distributes available energy from the pool to tenants with energy demand,
        based on their reputation or equally if no reputation is present.
        """
